...
AMCAM_FLAG_HIGH_FULLWELL       = 0x00000800          # high fullwell capacity
...
AMCAM_FLAG_DDR                 = 0x02000000          # use very large capacity DDR (Double Data Rate SDRAM) for frame buffer. The capacity is not less than one full frame
...
AMCAM_FLAG_LIGHTSOURCE         = 0x0000080000000000  # embedded light source
AMCAM_FLAG_FILTERWHEEL         = 0x0000100000000000  # astro filter wheel
AMCAM_FLAG_GIGE                = 0x0000200000000000  # 1 Gigabit GigE
AMCAM_FLAG_10GIGE              = 0x0000400000000000  # 10 Gigabit GigE
AMCAM_FLAG_5GIGE               = 0x0000800000000000  # 5 Gigabit GigE
AMCAM_FLAG_25GIGE              = 0x0001000000000000  # 2.5 Gigabit GigE
AMCAM_FLAG_AUTOFOCUSER         = 0x0002000000000000  # astro auto focuser
AMCAM_FLAG_LIGHT_SOURCE        = 0x0004000000000000  # stand alone light source
AMCAM_FLAG_CAMERALINK          = 0x0008000000000000  # camera link
AMCAM_FLAG_CXP                 = 0x0010000000000000  # CXP: CoaXPress
AMCAM_FLAG_RAW12PACK           = 0x0020000000000000  # pixel format, RAW 12bits packed
AMCAM_FLAG_SELFTRIGGER         = 0x0040000000000000  # self trigger
AMCAM_FLAG_RAW11               = 0x0080000000000000  # pixel format, RAW 11bits
AMCAM_FLAG_GHOPTO              = 0x0100000000000000  # ghopto sensor
...
AMCAM_EVENT_IMAGE              = 0x0004          # live image arrived, use PullImageXXXX to get this image
AMCAM_EVENT_STILLIMAGE         = 0x0005          # snap (still) frame arrived, use PullStillImageXXXX to get this frame
...
AMCAM_EVENT_LEVELRANGE         = 0x000c          # level range changed
AMCAM_EVENT_AUTOEXPO_CONV      = 0x000d          # auto exposure convergence
AMCAM_EVENT_AUTOEXPO_CONVFAIL  = 0x000e          # auto exposure once mode convergence failed
...
AMCAM_EVENT_AFFEEDBACK         = 0x0083          # auto focus information feedback
AMCAM_EVENT_FOCUSPOS           = 0x0084          # focus positon
AMCAM_EVENT_NOPACKETTIMEOUT    = 0x0085          # no packet timeout
AMCAM_EVENT_EXPO_START         = 0x4000          # hardware event: exposure start
AMCAM_EVENT_EXPO_STOP          = 0x4001          # hardware event: exposure stop
AMCAM_EVENT_TRIGGER_ALLOW      = 0x4002          # hardware event: next trigger allow
AMCAM_EVENT_HEARTBEAT          = 0x4003          # hardware event: heartbeat, can be used to monitor whether the camera is alive
AMCAM_EVENT_TRIGGER_IN         = 0x4004          # hardware event: trigger in
AMCAM_EVENT_FACTORY            = 0x8001          # restore factory settings

AMCAM_OPTION_NOFRAME_TIMEOUT        = 0x01       # no frame timeout: 0 => disable, positive value (>= NOFRAME_TIMEOUT_MIN) => timeout milliseconds. default: disable
AMCAM_OPTION_THREAD_PRIORITY        = 0x02       # set the priority of the internal thread which grab data from the usb device.
                                                   ...
                                                   #
AMCAM_OPTION_RAW                    = 0x04       # raw data mode, read the sensor "raw" data. This can be set only while camea is NOT running. 0 = rgb, 1 = raw, default value: 0
...
AMCAM_OPTION_RGB                    = 0x0c       # 0 => RGB24; 1 => enable RGB48 format when bitdepth > 8; 2 => RGB32; 3 => 8 Bits Grey (only for mono camera); 4 => 16 Bits Grey (only for mono camera when bitdepth > 8); 5 => 64(RGB64)
...
AMCAM_OPTION_BINNING                = 0x17       # binning
                                                   #     0x01: (no binning)
...
AMCAM_OPTION_CG                     = 0x19       # Conversion Gain:
                                                   #     0 = LCG
                                                   #     1 = HCG
                                                   #     2 = HDR (for camera with flag AMCAM_FLAG_CGHDR)
                                                   #     2 = MCG (for camera with flag AMCAM_FLAG_GHOPTO)
                                                   #
...
AMCAM_OPTION_TEC_VOLTAGE_MAX        = 0x21       # TEC maximum voltage in 0.1V
...
AMCAM_OPTION_UPSIDE_DOWN            = 0x23       # upsize down:
                                                   ...
                                                   #
AMCAM_OPTION_FOCUSPOS               = 0x24       # focus positon
...
AMCAM_OPTION_NOPACKET_TIMEOUT       = 0x2b       # no packet timeout: 0 => disable, positive value (>= NOPACKET_TIMEOUT_MIN) => timeout milliseconds. default: disable
AMCAM_OPTION_MAX_PRECISE_FRAMERATE  = 0x2c       # get the precise frame rate maximum value in 0.1 fps, such as 115 means 11.5 fps. E_NOTIMPL means not supported
...
AMCAM_OPTION_FRONTEND_DEQUE_LENGTH  = 0x31       # frontend (raw) frame buffer deque length, range: [2, 1024], default: 4
...
AMCAM_OPTION_MIN_PRECISE_FRAMERATE  = 0x32       # get the precise frame rate minimum value in 0.1 fps, such as 15 means 1.5 fps
...
AMCAM_OPTION_OPEN_ERRORCODE         = 0x3b       # get the open camera error code
...
AMCAM_OPTION_DUMP_CFG               = 0x3f       # 0 = when camera is stopped, do not dump configuration automatically
...
AMCAM_OPTION_BACKEND_DEQUE_LENGTH   = 0x41       # backend (pipelined) frame buffer deque length (Only available in pull mode), range: [2, 1024], default: 3
                                                   # All the memory will be pre-allocated when the camera starts, so, please attention to memory usage
                                                   #
AMCAM_OPTION_LIGHTSOURCE_MAX        = 0x42       # get the light source range, [0 ~ max]
AMCAM_OPTION_LIGHTSOURCE            = 0x43       # light source
AMCAM_OPTION_HEARTBEAT              = 0x44       # Heartbeat interval in millisecond, range = [AMCAM_HEARTBEAT_MIN, AMCAM_HEARTBEAT_MAX], 0 = disable, default: disable
AMCAM_OPTION_FRONTEND_DEQUE_CURRENT = 0x45       # get the current number in frontend deque
AMCAM_OPTION_BACKEND_DEQUE_CURRENT  = 0x46       # get the current number in backend deque
AMCAM_OPTION_EVENT_HARDWARE         = 0x04000000 # enable or disable hardware event: 0 => disable, 1 => enable; default: disable
                                                   #     (1) iOption = AMCAM_OPTION_EVENT_HARDWARE, master switch for notification of all hardware events
                                                   #     (2) iOption = AMCAM_OPTION_EVENT_HARDWARE | (event type), a specific type of sub-switch
                                                   ...
                                                   #
AMCAM_OPTION_PACKET_NUMBER          = 0x47       # get the received packet number
AMCAM_OPTION_FILTERWHEEL_SLOT       = 0x48       # filter wheel slot number
AMCAM_OPTION_FILTERWHEEL_POSITION   = 0x49       # filter wheel position:
                                                   #     set:
                                                   #         -1: calibrate
                                                   ...
                                                   #
AMCAM_OPTION_AUTOEXPOSURE_PERCENT   = 0x4a       # auto exposure percent to average:
                                                   #     1~99: peak percent average
                                                   #     0 or 100: full roi average, means "disabled"
                                                   #
AMCAM_OPTION_ANTI_SHUTTER_EFFECT    = 0x4b       # anti shutter effect: 1 => disable, 0 => disable; default: 0
AMCAM_OPTION_CHAMBER_HT             = 0x4c       # get chamber humidity & temperature:
                                                   ...
                                                   #
AMCAM_OPTION_ENV_HT                 = 0x4d       # get environment humidity & temperature
AMCAM_OPTION_EXPOSURE_PRE_DELAY     = 0x4e       # exposure signal pre-delay, microsecond
AMCAM_OPTION_EXPOSURE_POST_DELAY    = 0x4f       # exposure signal post-delay, microsecond
AMCAM_OPTION_AUTOEXPO_CONV          = 0x50       # get auto exposure convergence status: 1(YES) or 0(NO), -1(NA)
AMCAM_OPTION_AUTOEXPO_TRIGGER       = 0x51       # auto exposure on trigger mode: 0 => disable, 1 => enable; default: 0
AMCAM_OPTION_LINE_PRE_DELAY         = 0x52       # specified line signal pre-delay, microsecond
AMCAM_OPTION_LINE_POST_DELAY        = 0x53       # specified line signal post-delay, microsecond
AMCAM_OPTION_TEC_VOLTAGE_MAX_RANGE  = 0x54       # get the tec maximum voltage range:
                                                   #      high 16 bits: max
                                                   #      low 16 bits: min
AMCAM_OPTION_HIGH_FULLWELL          = 0x55       # high fullwell capacity: 0 => disable, 1 => enable
AMCAM_OPTION_DYNAMIC_DEFECT         = 0x56       # dynamic defect pixel correction:
                                                   #      threshold, t1: high 16 bits: [10, 100], means: [1.0, 10.0]
                                                   #      value, t2: low 16 bits: [0, 100], means: [0.00, 1.00]
AMCAM_OPTION_HDR_KB                 = 0x57       # HDR synthesize
                                                   #      K (high 16 bits): [1, 25500]
                                                   #      B (low 16 bits): [0, 65535]
                                                   #      0xffffffff => set to default
AMCAM_OPTION_HDR_THRESHOLD          = 0x58       # HDR synthesize
                                                   #      threshold: [1, 4094]
                                                   #      0xffffffff => set to default
AMCAM_OPTION_GIGETIMEOUT            = 0x5a       # For GigE cameras, the application periodically sends heartbeat signals to the camera to keep the connection to the camera alive.
                                                   ...
                                                   #     2 => disable
                                                   #     default: auto
AMCAM_OPTION_EEPROM_SIZE            = 0x5b       # get EEPROM size
AMCAM_OPTION_OVERCLOCK_MAX          = 0x5c       # get overclock range: [0, max]
AMCAM_OPTION_OVERCLOCK              = 0x5d       # overclock, default: 0
AMCAM_OPTION_RESET_SENSOR           = 0x5e       # reset sensor
AMCAM_OPTION_ISP                    = 0x5f       # Enable hardware ISP: 0 => auto (disable in RAW mode, otherwise enable), 1 => enable, -1 => disable; default: 0
AMCAM_OPTION_AUTOEXP_EXPOTIME_STEP  = 0x60       # Auto exposure: time step (thousandths)
AMCAM_OPTION_AUTOEXP_GAIN_STEP      = 0x61       # Auto exposure: gain step (thousandths)
AMCAM_OPTION_MOTOR_NUMBER           = 0x62       # range: [1, 20]
AMCAM_OPTION_MOTOR_POS              = 0x10000000 # range: [1, 702]
AMCAM_OPTION_PSEUDO_COLOR_START     = 0x63       # Pseudo: start color, BGR format
AMCAM_OPTION_PSEUDO_COLOR_END       = 0x64       # Pseudo: end color, BGR format
AMCAM_OPTION_PSEUDO_COLOR_ENABLE    = 0x65       # Pseudo: -1 => custom: use startcolor & endcolor to generate the colormap
                                                   #          0 => disable
                                                   #          1 => spot
                                                   #          2 => spring
                                                   #          3 => summer
...
AMCAM_PIXELFORMAT_GMCY8             = 0x09   # map to RGGB 8 bits
AMCAM_PIXELFORMAT_GMCY12            = 0x0a   # map to RGGB 12 bits
AMCAM_PIXELFORMAT_UYVY              = 0x0b
AMCAM_PIXELFORMAT_RAW12PACK         = 0x0c
AMCAM_PIXELFORMAT_RAW11             = 0x0d
AMCAM_PIXELFORMAT_HDR8HL            = 0x0e   # HDR, Bitdepth: 8, Conversion Gain: High + Low
AMCAM_PIXELFORMAT_HDR10HL           = 0x0f   # HDR, Bitdepth: 10, Conversion Gain: High + Low
AMCAM_PIXELFORMAT_HDR11HL           = 0x10   # HDR, Bitdepth: 11, Conversion Gain: High + Low
AMCAM_PIXELFORMAT_HDR12HL           = 0x11   # HDR, Bitdepth: 12, Conversion Gain: High + Low
AMCAM_PIXELFORMAT_HDR14HL           = 0x12   # HDR, Bitdepth: 14, Conversion Gain: High + Low

AMCAM_FRAMEINFO_FLAG_SEQ            = 0x00000001   # frame sequence number
AMCAM_FRAMEINFO_FLAG_TIMESTAMP      = 0x00000002   # timestamp
AMCAM_FRAMEINFO_FLAG_EXPOTIME       = 0x00000004   # exposure time
AMCAM_FRAMEINFO_FLAG_EXPOGAIN       = 0x00000008   # exposure gain
AMCAM_FRAMEINFO_FLAG_BLACKLEVEL     = 0x00000010   # black level
AMCAM_FRAMEINFO_FLAG_SHUTTERSEQ     = 0x00000020   # sequence shutter counter
AMCAM_FRAMEINFO_FLAG_STILL          = 0x00008000   # still image
...
AMCAM_IOCONTROLTYPE_GET_INPUTACTIVATION       = 0x09  # 0x00 => Rising edge, 0x01 => Falling edge, 0x02 => Level high, 0x03 => Level low
AMCAM_IOCONTROLTYPE_SET_INPUTACTIVATION       = 0x0a
AMCAM_IOCONTROLTYPE_GET_DEBOUNCERTIME         = 0x0b  # debouncer time in microseconds, range: [0, 20000]
...
AMCAM_IOCONTROLTYPE_GET_TRIGGERDELAY          = 0x0f  # Trigger delay time in microseconds, range: [0, 5000000]
AMCAM_IOCONTROLTYPE_SET_TRIGGERDELAY          = 0x10
AMCAM_IOCONTROLTYPE_GET_BURSTCOUNTER          = 0x11  # Burst Counter, range: [1 ~ 65535]
...
AMCAM_IOCONTROLTYPE_GET_COUNTERVALUE          = 0x15  # Counter Value, range: [1 ~ 65535]
...
AMCAM_IOCONTROLTYPE_GET_STROBEDELAYTIME       = 0x23  # Strobe delay or pre-delay time in microseconds, range: [0, 5000000]
AMCAM_IOCONTROLTYPE_SET_STROBEDELAYTIME       = 0x24
AMCAM_IOCONTROLTYPE_GET_STROBEDURATION        = 0x25  # Strobe duration time in microseconds, range: [0, 5000000]
...
AMCAM_IOCONTROLTYPE_GET_EXPO_ACTIVE_MODE      = 0x2f  # exposure time signal: 0 => specified line, 1 => common exposure time
AMCAM_IOCONTROLTYPE_SET_EXPO_ACTIVE_MODE      = 0x30
AMCAM_IOCONTROLTYPE_GET_EXPO_START_LINE       = 0x31  # exposure start line, default: 0
AMCAM_IOCONTROLTYPE_SET_EXPO_START_LINE       = 0x32
AMCAM_IOCONTROLTYPE_GET_EXPO_END_LINE         = 0x33  # exposure end line, default: 0
                                                        # end line must be no less than start line
AMCAM_IOCONTROLTYPE_SET_EXPO_END_LINE         = 0x34
AMCAM_IOCONTROLTYPE_GET_EXEVT_ACTIVE_MODE     = 0x35  # exposure event: 0 => specified line, 1 => common exposure time
AMCAM_IOCONTROLTYPE_SET_EXEVT_ACTIVE_MODE     = 0x36
AMCAM_IOCONTROLTYPE_GET_OUTPUTCOUNTERVALUE    = 0x37  # Output Counter Value, range: [0 ~ 65535]
AMCAM_IOCONTROLTYPE_SET_OUTPUTCOUNTERVALUE    = 0x38
AMCAM_IOCONTROLTYPE_SET_OUTPUT_PAUSE          = 0x3a  # Output pause: 1 => puase, 0 => unpause

# AAF: Astro Auto Focuser
AMCAM_AAF_SETPOSITION     = 0x01
AMCAM_AAF_GETPOSITION     = 0x02
AMCAM_AAF_SETZERO         = 0x03
AMCAM_AAF_GETZERO         = 0x04
AMCAM_AAF_SETDIRECTION    = 0x05
AMCAM_AAF_SETMAXINCREMENT = 0x07
AMCAM_AAF_GETMAXINCREMENT = 0x08
AMCAM_AAF_SETFINE         = 0x09
AMCAM_AAF_GETFINE         = 0x0a
AMCAM_AAF_SETCOARSE       = 0x0b
AMCAM_AAF_GETCOARSE       = 0x0c
AMCAM_AAF_SETBUZZER       = 0x0d
AMCAM_AAF_GETBUZZER       = 0x0e
AMCAM_AAF_SETBACKLASH     = 0x0f
AMCAM_AAF_GETBACKLASH     = 0x10
AMCAM_AAF_GETAMBIENTTEMP  = 0x12
AMCAM_AAF_GETTEMP         = 0x14  # in 0.1 degrees Celsius, such as: 32 means 3.2 degrees Celsius
AMCAM_AAF_ISMOVING        = 0x16
AMCAM_AAF_HALT            = 0x17
AMCAM_AAF_SETMAXSTEP      = 0x1b
AMCAM_AAF_GETMAXSTEP      = 0x1c
AMCAM_AAF_GETSTEPSIZE     = 0x1e
AMCAM_AAF_RANGEMIN        = 0xfd  # Range: min value
AMCAM_AAF_RANGEMAX        = 0xfe  # Range: max value
AMCAM_AAF_RANGEDEF        = 0xff  # Range: default value
...
# see rwc_Flash
AMCAM_FLASH_SIZE      = 0x00    # query total size
AMCAM_FLASH_EBLOCK    = 0x01    # query erase block size
AMCAM_FLASH_RWBLOCK   = 0x02    # query read/write block size
AMCAM_FLASH_STATUS    = 0x03    # query status
AMCAM_FLASH_READ      = 0x04    # read
AMCAM_FLASH_WRITE     = 0x05    # write
AMCAM_FLASH_ERASE     = 0x06    # erase
...
AMCAM_EXPOGAIN_DEF             = 100      # exposure gain, default value
AMCAM_EXPOGAIN_MIN             = 100      # exposure gain, minimum value
AMCAM_TEMP_DEF                 = 6503     # color temperature, default value
AMCAM_TEMP_MIN                 = 2000     # color temperature, minimum value
AMCAM_TEMP_MAX                 = 15000    # color temperature, maximum value
AMCAM_TINT_DEF                 = 1000     # tint
AMCAM_TINT_MIN                 = 200      # tint
AMCAM_TINT_MAX                 = 2500     # tint
AMCAM_HUE_DEF                  = 0        # hue
AMCAM_HUE_MIN                  = -180     # hue
AMCAM_HUE_MAX                  = 180      # hue
AMCAM_SATURATION_DEF           = 128      # saturation
AMCAM_SATURATION_MIN           = 0        # saturation
AMCAM_SATURATION_MAX           = 255      # saturation
AMCAM_BRIGHTNESS_DEF           = 0        # brightness
AMCAM_BRIGHTNESS_MIN           = -64      # brightness
AMCAM_BRIGHTNESS_MAX           = 64       # brightness
AMCAM_CONTRAST_DEF             = 0        # contrast
AMCAM_CONTRAST_MIN             = -100     # contrast
AMCAM_CONTRAST_MAX             = 100      # contrast
AMCAM_GAMMA_DEF                = 100      # gamma
AMCAM_GAMMA_MIN                = 20       # gamma
AMCAM_GAMMA_MAX                = 180      # gamma
AMCAM_AETARGET_DEF             = 120      # target of auto exposure
AMCAM_AETARGET_MIN             = 16       # target of auto exposure
AMCAM_AETARGET_MAX             = 220      # target of auto exposure
AMCAM_WBGAIN_DEF               = 0        # white balance gain
AMCAM_WBGAIN_MIN               = -127     # white balance gain
AMCAM_WBGAIN_MAX               = 127      # white balance gain
AMCAM_BLACKLEVEL_MIN           = 0        # minimum black level
AMCAM_BLACKLEVEL8_MAX          = 31       # maximum black level for bitdepth = 8
AMCAM_BLACKLEVEL10_MAX         = 31 * 4   # maximum black level for bitdepth = 10
AMCAM_BLACKLEVEL11_MAX         = 31 * 8   # maximum black level for bitdepth = 11
AMCAM_BLACKLEVEL12_MAX         = 31 * 16  # maximum black level for bitdepth = 12
AMCAM_BLACKLEVEL14_MAX         = 31 * 64  # maximum black level for bitdepth = 14
AMCAM_BLACKLEVEL16_MAX         = 31 * 256 # maximum black level for bitdepth = 16
AMCAM_SHARPENING_STRENGTH_DEF  = 0        # sharpening strength
AMCAM_SHARPENING_STRENGTH_MIN  = 0        # sharpening strength
AMCAM_SHARPENING_STRENGTH_MAX  = 500      # sharpening strength
AMCAM_SHARPENING_RADIUS_DEF    = 2        # sharpening radius
AMCAM_SHARPENING_RADIUS_MIN    = 1        # sharpening radius
AMCAM_SHARPENING_RADIUS_MAX    = 10       # sharpening radius
AMCAM_SHARPENING_THRESHOLD_DEF = 0        # sharpening threshold
AMCAM_SHARPENING_THRESHOLD_MIN = 0        # sharpening threshold
AMCAM_SHARPENING_THRESHOLD_MAX = 255      # sharpening threshold
AMCAM_AUTOEXPO_THRESHOLD_DEF   = 5        # auto exposure threshold
AMCAM_AUTOEXPO_THRESHOLD_MIN   = 2        # auto exposure threshold
AMCAM_AUTOEXPO_THRESHOLD_MAX   = 15       # auto exposure threshold
AMCAM_AUTOEXPO_STEP_DEF        = 1000     # auto exposure step: thousandths
AMCAM_AUTOEXPO_STEP_MIN        = 1        # auto exposure step: thousandths
AMCAM_AUTOEXPO_STEP_MAX        = 1000     # auto exposure step: thousandths
AMCAM_BANDWIDTH_DEF            = 100      # bandwidth
AMCAM_BANDWIDTH_MIN            = 1        # bandwidth
AMCAM_BANDWIDTH_MAX            = 100      # bandwidth
AMCAM_DENOISE_DEF              = 0        # denoise
AMCAM_DENOISE_MIN              = 0        # denoise
AMCAM_DENOISE_MAX              = 100      # denoise
AMCAM_TEC_TARGET_MIN           = -500     # TEC target: -50.0 degrees Celsius
AMCAM_TEC_TARGET_DEF           = 100      # TEC target: 0.0 degrees Celsius
AMCAM_TEC_TARGET_MAX           = 400      # TEC target: 40.0 degrees Celsius
AMCAM_HEARTBEAT_MIN            = 100      # millisecond
AMCAM_HEARTBEAT_MAX            = 10000    # millisecond
AMCAM_AE_PERCENT_MIN           = 0        # auto exposure percent; 0 or 100 => full roi average, means "disabled"
AMCAM_AE_PERCENT_MAX           = 100
AMCAM_AE_PERCENT_DEF           = 10       # auto exposure percent: enabled, percentage = 10%
AMCAM_NOPACKET_TIMEOUT_MIN     = 500      # no packet timeout minimum: 500ms
AMCAM_NOFRAME_TIMEOUT_MIN      = 500      # no frame timeout minimum: 500ms
AMCAM_DYNAMIC_DEFECT_T1_MIN    = 10       # dynamic defect pixel correction, threshold, means: 1.0
AMCAM_DYNAMIC_DEFECT_T1_MAX    = 100      # means: 10.0
AMCAM_DYNAMIC_DEFECT_T1_DEF    = 13       # means: 1.3
AMCAM_DYNAMIC_DEFECT_T2_MIN    = 0        # dynamic defect pixel correction, value, means: 0.00
AMCAM_DYNAMIC_DEFECT_T2_MAX    = 100      # means: 1.00
AMCAM_DYNAMIC_DEFECT_T2_DEF    = 100
AMCAM_HDR_K_MIN                = 1        # HDR synthesize
AMCAM_HDR_K_MAX                = 25500
AMCAM_HDR_B_MIN                = 0
AMCAM_HDR_B_MAX                = 65535
AMCAM_HDR_THRESHOLD_MIN        = 0
AMCAM_HDR_THRESHOLD_MAX        = 4094
...
class AmcamFrameInfoV3:
    def __init__(self):
        self.width = 0
        self.height = 0
        self.flag = 0                    # AMCAM_FRAMEINFO_FLAG_xxxx
...
class Amcam:
    ...
    @classmethod
    def Version(cls):
        """get the version of this dll, which is: 55.24172.20231217"""
        cls.__initlib()
    ...
    def GigeEnable(cls, fun, ctx):
        ...
        if cls.__gigeenable_fun is None:
            cls.__lib.Amcam_GigeEnable(cls.__HOTPLUG_CALLBACK(0), None)
        else:
            cls.__gigeenable_cb = cls.__HOTPLUG_CALLBACK(cls.__gigeEnableCallbackFun)
            cls.__lib.Amcam_GigeEnable(cls.__gigeenable_cb, None)
    ...
    def HotPlug(cls, fun, ctx):
        ...
        USB hotplug is only available on macOS and Linux, it's unnecessary on Windows & Android. To process the device plug in / pull out:
            ...
            (3) On Linux / macOS, please call this function to register the callback function.
                When the device is inserted or pulled out, you will be notified by the callback funcion, and then call Amcam_EnumV2(...) again to enum the cameras.
        ...
        if sys.platform == 'win32' or sys.platform == 'android':
            ...
            if cls.__hotplug_fun is None:
                cls.__lib.Amcam_HotPlug(cls.__HOTPLUG_CALLBACK(0), None)
            else:
                cls.__hotplug_cb = cls.__HOTPLUG_CALLBACK(cls.__hotplugCallbackFun)
                cls.__lib.Amcam_HotPlug(__hotplug_cb, None)
    ...
    def EnumWithName(cls):
        cls.__initlib()
        a = (_DeviceV2 * AMCAM_MAX)()
        n = cls.__lib.Amcam_EnumWithName(a)
        arr = []
        for i in range(0, n):
            arr.append(cls.__convertDevice(a[i]))
        return arr
    ...
    def Open(cls, camId):
        ...
        if camId is None:
            ...
            h = cls.__lib.Amcam_Open(camId)
        else:
            h = cls.__lib.Amcam_Open(camId.encode('ascii'))
        if h is None:
            return None
        return __class__(h)
    ...
    """
        ...
        bits: 24 (RGB24), 32 (RGB32), 48 (RGB48), 8 (Grey), 16 (Grey), 64 (RGB64).
              ...
              bits = 0 means using default bits (see AMCAM_OPTION_RGB).
              When bits and AMCAM_OPTION_RGB are inconsistent, format conversion will have to be performed, resulting in loss of efficiency.
              See the following bits and AMCAM_OPTION_RGB correspondence table:
                ----------------------------------------------------------------------------------------------------------------------
                | AMCAM_OPTION_RGB |   0 (RGB24)   |   1 (RGB48)   |   2 (RGB32)   |   3 (Grey8)   |  4 (Grey16)   |   5 (RGB64)   |
                |--------------------|---------------|---------------|---------------|---------------|---------------|---------------|
    ...
    def PullImageV3(self, pImageData, bStill, bits, rowPitch, pInfo):
        if pInfo is None:
            self.__lib.Amcam_PullImageV3(self.__h, pImageData, bStill, bits, rowPitch, None)
        else:
            x = self.__FrameInfoV3()
            self.__lib.Amcam_PullImageV3(self.__h, pImageData, bStill, bits, rowPitch, ctypes.byref(x))
            self.__convertFrameInfoV3(pInfo, x)

    def WaitImageV3(self, nWaitMS, pImageData, bStill, bits, rowPitch, pInfo):
        if pInfo is None:
            self.__lib.Amcam_WaitImageV3(self.__h, nWaitMS, pImageData, bStill, bits, rowPitch, None)
        else:
            x = self.__FrameInfoV3()
            self.__lib.Amcam_WaitImageV3(self.__h, nWaitMS, pImageData, bStill, bits, rowPitch, ctypes.byref(x))
            self.__convertFrameInfoV3(pInfo, x)
    ...
    def PullImageV2(self, pImageData, bits, pInfo):
        if pInfo is None:
            ...
            self.__lib.Amcam_PullImageV2(self.__h, pImageData, bits, ctypes.byref(x))
    ...
    def PullStillImageV2(self, pImageData, bits, pInfo):
        if pInfo is None:
            ...
            x = self.__FrameInfoV2()
            self.__lib.Amcam_PullStillImageV2(self.__h, pImageData, bits, ctypes.byref(x))
            self.__convertFrameInfoV2(pInfo, x)
    ...
    def PullImageWithRowPitchV2(self, pImageData, bits, rowPitch, pInfo):
        if pInfo is None:
            ...
            x = self.__FrameInfoV2()
            self.__lib.Amcam_PullImageWithRowPitchV2(self.__h, pImageData, bits, rowPitch, ctypes.byref(x))
            self.__convertFrameInfoV2(pInfo, x)
    ...
    def PullStillImageWithRowPitchV2(self, pImageData, bits, rowPitch, pInfo):
        if pInfo is None:
            ...
            x = self.__FrameInfoV2()
            self.__lib.Amcam_PullStillImageWithRowPitchV2(self.__h, pImageData, bits, rowPitch, ctypes.byref(x))
            self.__convertFrameInfoV2(pInfo, x)
    ...
    def SnapR(self, nResolutionIndex, nNumber):
        ...
        self.__lib.Amcam_SnapR(self.__h, ctypes.c_uint(nResolutionIndex), ctypes.c_uint(nNumber))
    ...
    def Flush(self):
        """Flush is obsolete, recommend using put_Option(h, AMCAM_OPTION_FLUSH, 3)"""
    ...
    def put_AutoExpoEnable(self, bAutoExposure):
        ...
        bAutoExposure:
           ...
           2: auto exposure once mode
        """
        self.__lib.Amcam_put_AutoExpoEnable(self.__h, ctypes.c_int(bAutoExposure))

    def get_AutoExpoTarget(self):
        x = ctypes.c_ushort(AMCAM_AETARGET_DEF)
    ...
    def put_AutoExpoRange(self, maxTime, minTime, maxGain, minGain):
        return self.__lib.Amcam_put_AutoExpoRange(self.__h, ctypes.c_uint(maxTime), ctypes.c_uint(minTime), ctypes.c_ushort(maxGain), ctypes.c_ushort(minGain))
    ...
    def get_AutoExpoRange(self):
        ...
        self.__lib.Amcam_get_AutoExpoRange(self.__h, ctypes.byref(maxTime), ctypes.byref(minTime), ctypes.byref(maxGain), ctypes.byref(minGain))
        return (maxTime.value, minTime.value, maxGain.value, minGain.value)

    def put_MaxAutoExpoTimeAGain(self, maxTime, maxGain):
        return self.__lib.Amcam_put_MaxAutoExpoTimeAGain(self.__h, ctypes.c_uint(maxTime), ctypes.c_ushort(maxGain))
    ...
    def put_MinAutoExpoTimeAGain(self, minTime, minGain):
        self.__lib.Amcam_put_MinAutoExpoTimeAGain(self.__h, ctypes.c_uint(minTime), ctypes.c_ushort(minGain))
    ...
    def put_ExpoAGain(self, Gain):
        self.__lib.Amcam_put_ExpoAGain(self.__h, ctypes.c_ushort(Gain))
    ...
    def get_Saturation(self):
        x = ctypes.c_int(AMCAM_SATURATION_DEF)
    ...
    def get_Brightness(self):
        x = ctypes.c_int(AMCAM_BRIGHTNESS_DEF)
        self.__lib.Amcam_get_Brightness(self.__h, ctypes.byref(x))
        return x.value

    def get_Contrast(self):
        x = ctypes.c_int(AMCAM_CONTRAST_DEF)
    ...
    def get_Gamma(self):
        x = ctypes.c_int(AMCAM_GAMMA_DEF)
    ...
    def get_TempTint(self):
        """White Balance, Temp/Tint mode"""
        x = ctypes.c_int(AMCAM_TEMP_DEF)
        y = ctypes.c_int(AMCAM_TINT_DEF)
    ...
    def rwc_Flash(self, action, addr, pData):
        """
        Flash:
        action = AMCAM_FLASH_XXXX: read, write, erase, query total size, query read/write block size, query erase block size
        addr = address
        see democpp
        """
        self.__lib.Amcam_rwc_Flash(self.__h, action, addr, ctypes.c_uint(len(pData)), pData)

    def write_Pipe(self, pipeId, pBuffer):
        self.__lib.Amcam_write_Pipe(self.__h, pipeId, pBuffer, ctypes.c_uint(len(pBuffer)))

    def read_Pipe(self, pipeId, pBuffer):
        self.__lib.Amcam_read_Pipe(self.__h, pipeId, pBuffer, ctypes.c_uint(len(pBuffer)))

    def feed_Pipe(self, pipeId):
        self.__lib.Amcam_feed_Pipe(self.__h, ctypes.c_uint(pipeId))
    ...
    def get_PixelFormatSupport(self, cmd):
        ...
        output: AMCAM_PIXELFORMAT_xxxx
        """
        x = ctypes.c_int(0)
        self.__lib.Amcam_get_PixelFormatSupport(self.__h, cmd, ctypes.byref(x))
        return x.value
    ...
    def put_XY(self, x, y):
        self.__lib.Amcam_put_XY(self.__h, ctypes.c_int(x), ctypes.c_int(y))
    ...
    def IoControl(self, ioLineNumber, eType, outVal):
        x = ctypes.c_int(0)
        self.__lib.Amcam_IoControl(self.__h, ctypes.c_uint(ioLineNumber), ctypes.c_uint(eType), ctypes.c_int(outVal), ctypes.byref(x))
        return x.value

    def AAF(self, action, outVal):
        x = ctypes.c_int(0)
        self.__lib.Amcam_AAF(self.__h, ctypes.c_int(action), ctypes.c_int(outVal), ctypes.byref(x))
        return x.value
    ...
    def GetHistogram(self, fun, ctx):
        ...
        self.__lib.Amcam_GetHistogramV2(self.__h, self.__cbhistogram, ctypes.py_object(self))

    @classmethod
    def PixelFormatName(cls, val):
        cls.__initlib()
        return cls.__lib.Amcam_get_PixelFormatName(val)
    ...
    def Replug(cls, camId):
        ...
        cls.__initlib()
        if sys.platform == 'win32':
            return cls.__lib.Amcam_Replug(camId)
        else:
            return cls.__lib.Amcam_Replug(camId.encode('ascii'))
    ...
    def Update(cls, camId, filePath, pFun, pCtx):
        ...
        if sys.platform == 'win32':
            return cls.__lib.Amcam_Update(camId, filePath, __progress_cb, None)
        else:
            return cls.__lib.Amcam_Update(camId.encode('ascii'), filePath.encode('ascii'), __progress_cb, None)
    ...
    def Gain2TempTint(cls, gain):
        ...
        if len(gain) == 3:
            ...
            temp = ctypes.c_int(AMCAM_TINT_DEF)
            tint = ctypes.c_int(AMCAM_TINT_DEF)
            cls.__lib.Amcam_Gain2TempTint(x, ctypes.byref(temp), ctypes.byref(tint))
    ...
    def TempTint2Gain(cls, temp, tint):
        cls.__initlib()
        x = (ctypes.c_int * 3)()
        cls.__lib.Amcam_TempTint2Gain(ctypes.c_int(temp), ctypes.c_int(tint), x)
        return (x[0], x[1], x[2])
    ...
    def __initlib(cls):
        if cls.__lib is None:
            ...
            cls.__lib.Amcam_EnumWithName.restype = ctypes.c_uint
            cls.__lib.Amcam_EnumWithName.argtypes = [_DeviceV2 * AMCAM_MAX]
            ...
            cls.__lib.Amcam_PullImageV3.restype = ctypes.c_int
            cls.__lib.Amcam_PullImageV3.errcheck = cls.__errcheck
            cls.__lib.Amcam_PullImageV3.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.POINTER(cls.__FrameInfoV3)]
            cls.__lib.Amcam_WaitImageV3.restype = ctypes.c_int
            cls.__lib.Amcam_WaitImageV3.errcheck = cls.__errcheck
            cls.__lib.Amcam_WaitImageV3.argtypes = [ctypes.c_void_p, ctypes.c_uint, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.POINTER(cls.__FrameInfoV3)]       
            ...
            cls.__lib.Amcam_SnapR.restype = ctypes.c_int
            cls.__lib.Amcam_SnapR.errcheck = cls.__errcheck
            cls.__lib.Amcam_SnapR.argtypes = [ctypes.c_void_p, ctypes.c_uint, ctypes.c_uint]
            ...
            cls.__lib.Amcam_TriggerSync.restype = ctypes.c_int
            cls.__lib.Amcam_TriggerSync.errcheck = cls.__errcheck
            cls.__lib.Amcam_TriggerSync.argtypes = [ctypes.c_void_p, ctypes.c_uint, ctypes.c_char_p, ctypes.c_int, ctypes.c_int, ctypes.POINTER(cls.__FrameInfoV3)]            
            ...
            cls.__lib.Amcam_put_AutoExpoRange.restype = ctypes.c_int
            cls.__lib.Amcam_put_AutoExpoRange.errcheck = cls.__errcheck
            cls.__lib.Amcam_put_AutoExpoRange.argtypes = [ctypes.c_void_p, ctypes.c_uint, ctypes.c_uint, ctypes.c_ushort, ctypes.c_ushort]
            cls.__lib.Amcam_get_AutoExpoRange.restype = ctypes.c_int
            cls.__lib.Amcam_get_AutoExpoRange.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_AutoExpoRange.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_uint), ctypes.POINTER(ctypes.c_uint), ctypes.POINTER(ctypes.c_ushort), ctypes.POINTER(ctypes.c_ushort)]
            ...
            cls.__lib.Amcam_put_BlackBalance.argtypes = [ctypes.c_void_p, (ctypes.c_ushort * 3)]
            cls.__lib.Amcam_get_BlackBalance.restype = ctypes.c_int
            cls.__lib.Amcam_get_BlackBalance.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_BlackBalance.argtypes = [ctypes.c_void_p, (ctypes.c_ushort * 3)]
            ...
            cls.__lib.Amcam_put_Temperature.argtypes = [ctypes.c_void_p, ctypes.c_short]
            cls.__lib.Amcam_get_Temperature.restype = ctypes.c_int
            cls.__lib.Amcam_get_Temperature.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_Temperature.argtypes = [ctypes.c_void_p, ctypes.POINTER(ctypes.c_short)]
            ...
            cls.__lib.Amcam_rwc_Flash.restype = ctypes.c_int
            cls.__lib.Amcam_rwc_Flash.errcheck = cls.__errcheck
            cls.__lib.Amcam_rwc_Flash.argtypes = [ctypes.c_void_p, ctypes.c_uint, ctypes.c_uint, ctypes.c_uint, ctypes.c_char_p]
            ...
            cls.__lib.Amcam_get_PixelFormatSupport.restype = ctypes.c_int
            cls.__lib.Amcam_get_PixelFormatSupport.errcheck = cls.__errcheck
            cls.__lib.Amcam_get_PixelFormatSupport.argtypes = [ctypes.c_void_p, ctypes.c_char, ctypes.POINTER(ctypes.c_int)]
            cls.__lib.Amcam_get_PixelFormatName.restype = ctypes.c_char_p
            cls.__lib.Amcam_get_PixelFormatName.argtypes = [ctypes.c_int]
            ...
            cls.__lib.Amcam_put_XY.restype = ctypes.c_int
            cls.__lib.Amcam_put_XY.errcheck = cls.__errcheck
            cls.__lib.Amcam_put_XY.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_int]            
            ...
            cls.__lib.Amcam_AAF.restype = ctypes.c_int
            cls.__lib.Amcam_AAF.errcheck = cls.__errcheck
            cls.__lib.Amcam_AAF.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.c_int, ctypes.POINTER(ctypes.c_int)]
            ...
            cls.__lib.Amcam_GetHistogramV2.restype = ctypes.c_int
            cls.__lib.Amcam_GetHistogramV2.errcheck = cls.__errcheck
            cls.__lib.Amcam_GetHistogramV2.argtypes = [ctypes.c_void_p, cls.__HISTOGRAM_CALLBACK, ctypes.py_object]
            cls.__lib.Amcam_GigeEnable.restype = ctypes.c_int
            cls.__lib.Amcam_GigeEnable.argtypes = [cls.__HOTPLUG_CALLBACK, ctypes.c_void_p]
            cls.__lib.Amcam_Gain2TempTint.restype = ctypes.c_int
            cls.__lib.Amcam_Gain2TempTint.errcheck = cls.__errcheck
            cls.__lib.Amcam_Gain2TempTint.argtypes = [ctypes.c_int * 3, ctypes.POINTER(ctypes.c_int), ctypes.POINTER(ctypes.c_int)];
            cls.__lib.Amcam_TempTint2Gain.restype = None
            cls.__lib.Amcam_TempTint2Gain.argtypes = [ctypes.c_int, ctypes.c_int, ctypes.c_int * 3];
